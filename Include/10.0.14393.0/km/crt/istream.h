...
public:
    ...
    inline istream& get(_Out_writes_z_(lim) char *,int lim,char ='\n');
    ...
    inline istream& getline(_Out_writes_(lim) char *,int lim,char ='\n');
    ...
    istream& read(_Out_writes_(n) char *,int n);
...
protected:
    ...
    istream& get(_Out_writes_z_(lim) char *, int lim, int);
     int do_ipfx(int);

private:
    istream(ios&);
    int getint(_Out_writes_(MAXLONGSIZ) char *);
    int getdouble(_Out_writes_(buflen) char *, int buflen);
    int _fGline;
    int x_gcount;
};
    ...
    inline istream& istream::get(_Out_writes_(_lim)  char * _b, int _lim, char _delim) { return get(        _b, _lim, (int)(unsigned char)_delim); }
    ...
    inline istream& istream::getline(_Out_writes_(_lim)  char * _b,int _lim,char _delim) { lock(); _fGline++; get(        _b, _lim, (int)(unsigned char)_delim); unlock(); return *this; }
