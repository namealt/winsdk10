...
__analysis_noreturn void __stdcall _com_issue_error(HRESULT);
	...
	: m_str(NULL), m_RefCount(1)
{
	...
	if (FAILED(UIntAdd(l1, l2, &l3)) ||
            FAILED(UIntMult(l3, sizeof(wchar_t), &l3)))
	{
		_com_issue_error(E_OUTOFMEMORY);
	}

	m_wstr = ::SysAllocStringByteLen(NULL, l3);
...
public:
	...
	//
	bool operator==(_In_ const VARIANT& varSrc) const throw(_com_error);
	bool operator==(_In_ const VARIANT* pSrc) const throw(_com_error);
...
//
inline bool _variant_t::operator==(_In_ const VARIANT& varSrc) const throw()
{
	return *this == &varSrc;
}
...
// Compare a _variant_t against a const VARIANT* for equality
//
inline bool _variant_t::operator==(_In_ const VARIANT* pSrc) const throw()
{
	if (this == pSrc) {
		return true;
	}
