...
public:
	...
	//
	template<typename _InterfacePtr> _com_ptr_t(_In_ _Literal_ const _InterfacePtr& p) throw(_com_error)
		: m_pInterface(NULL)
	{
		if (p) {
    ...
    //
    _com_ptr_t(_In_ LPSTR str) throw(_com_error)
		: m_pInterface(NULL)
    {
    ...
    // Make sure correct ctor is called
    //
    _com_ptr_t(_In_ LPWSTR str) throw(_com_error)
		: m_pInterface(NULL)
    {
		HRESULT hr = CreateInstance(str, NULL, CLSCTX_ALL);
	...
	//
	_When_(null != 0, __drv_reportError("This will issue an error if any value other than null is assigned to the object.  Please review this function call."))
	_com_ptr_t(_In_ int null) throw(_com_error)
		: m_pInterface(NULL)
	{
		if (null != 0) {
			_com_issue_error(E_POINTER);
	...
	//
	_com_ptr_t(_In_ const _com_ptr_t& cp) throw()
	...
	// Saves the interface.
	//
	_com_ptr_t(_In_ Interface* pInterface) throw()
	...
	// be AddRef()ed.
	//
	_com_ptr_t(_In_ Interface* pInterface, _In_ bool fAddRef) throw()
		: m_pInterface(pInterface)
	{
		if (fAddRef) {
			_AddRef();
	...
	//
	template<> _com_ptr_t(_In_ const _variant_t& varSrc) throw(_com_error)
		: m_pInterface(NULL)
	{
	...
	//
	explicit _com_ptr_t(_In_ const CLSID& clsid, _In_opt_ IUnknown* pOuter = NULL, _In_opt_ DWORD dwClsContext = CLSCTX_ALL) throw(_com_error)
		: m_pInterface(NULL)
	{
	...
	// the string.
	//
  explicit _com_ptr_t(_In_ LPCWSTR str, _In_opt_ IUnknown* pOuter = NULL, _In_opt_ DWORD dwClsContext = CLSCTX_ALL) throw(_com_error)
		: m_pInterface(NULL)
	{
	...
	// the string.
	//
	explicit _com_ptr_t(_In_ LPCSTR str, _In_opt_ IUnknown* pOuter = NULL, _In_opt_ DWORD dwClsContext = CLSCTX_ALL) throw(_com_error)
		: m_pInterface(NULL)
	{
		HRESULT hr = CreateInstance(str, pOuter, dwClsContext);
	...
	//
	template<typename _InterfacePtr> _com_ptr_t& operator=(_In_opt_ const _InterfacePtr& p) throw(_com_error)
	{
		HRESULT hr = _QueryInterface(p);
	...
	// Saves the interface.
	//
	_com_ptr_t& operator=(_In_opt_ Interface* pInterface) throw()
	{
	...
	//
	_com_ptr_t& operator=(_In_opt_ const _com_ptr_t& cp) throw()
	{
	...
	//
	_When_(null != 0, __drv_reportError("This function will issue an error if any value other than NULL is assigned to it."))
	_com_ptr_t& operator=(_In_opt_ int null) throw(_com_error)
	{
		if (null != 0) {
			_com_issue_error(E_POINTER);
		}
	...
	// Construct a pointer for a _variant_t object.
	//
	template<> _com_ptr_t& operator=(_In_opt_ const _variant_t& varSrc) throw(_com_error)
	{
		HRESULT hr = QueryStdInterfaces(varSrc);
	...
	//
	void Attach(_In_ Interface* pInterface) throw()
	{
		_Release();
		m_pInterface = pInterface;
	}
	...
	//
	void Attach(_In_ Interface* pInterface, _In_ bool fAddRef) throw()
	{
		_Release();
		m_pInterface = pInterface;
	...
	//
	template<typename _InterfacePtr> bool operator==(_In_opt_ _InterfacePtr p) throw(_com_error)
	{
	...
	//
	template<> bool operator==(_In_opt_ Interface* p) throw(_com_error)
	{
	...
	//
	template<> bool operator==(_In_opt_ _com_ptr_t& p) throw()
	{
	...
	//
	template<> bool operator==(_In_opt_ _When_(null != 0, __drv_reportError("This function will issue an error if any value other than NULL is assigned to it.")) 
                                  int null) throw(_com_error)
	{
		if (null != 0) {
			_com_issue_error(E_POINTER);
		}
	...
	// Compares 2 _com_ptr_t's
	//
	template<> bool operator!=(_In_opt_ _com_ptr_t& p) throw(_com_error)
	{
		return !(operator==(p));
	}
	...
	// For comparison to NULL
	//
	template<> bool operator!=(_In_opt_ int null) throw(_com_error)
	{
	...
	//
	_Check_return_
	HRESULT CreateInstance(_In_ const CLSID& rclsid, _In_opt_ IUnknown* pOuter = NULL, _In_opt_ DWORD dwClsContext = CLSCTX_ALL) throw()
	{
		HRESULT hr;
	...
	//
	_Check_return_
  HRESULT CreateInstance(_In_ LPCWSTR clsidString, _In_opt_ IUnknown* pOuter = NULL, _In_opt_ DWORD dwClsContext = CLSCTX_ALL) throw()
    {
		if (clsidString == NULL) {
			return E_INVALIDARG;
		}
	...
	//
	_Check_return_
	HRESULT CreateInstance(_In_ LPCSTR clsidStringA, _In_opt_ IUnknown* pOuter = NULL, _In_opt_ DWORD dwClsContext = CLSCTX_ALL) throw()
	{
	...
	//
	_Check_return_
	HRESULT GetActiveObject(_In_ const CLSID& rclsid) throw()
	{
		_Release();
	...
	//
  _Check_return_
  HRESULT GetActiveObject(_In_ LPCWSTR clsidString) throw()
    {
		if (clsidString == NULL) {
			return E_INVALIDARG;
		}
	...
	//
	_Check_return_
	HRESULT GetActiveObject(_In_ LPCSTR clsidStringA) throw()
	{
		if (clsidStringA == NULL) {
			return E_INVALIDARG;
		}
	...
	//
	template<typename _InterfaceType> _Check_return_ HRESULT QueryInterface(_In_ const IID& iid, _In_ _InterfaceType*& p) throw ()
	{
		if (m_pInterface != NULL) {
...
private:
	...
	//
	template<typename _InterfacePtr> _Check_return_ HRESULT _QueryInterface(_In_ const _InterfacePtr& p) throw()
	{
		HRESULT hr;
	...
	//
	template<typename _InterfacePtr> int _CompareUnknown(_In_opt_ _InterfacePtr p) throw(_com_error)
	{
		IUnknown* pu1, *pu2;
	...
	//
	_Check_return_
	HRESULT QueryStdInterfaces(_In_ const _variant_t& varSrc) throw()
	{
...
//
template<typename _InterfaceType> bool operator==(_In_opt_ _When_(null != 0, __drv_reportError("This function will issue an error if any value other than NULL is assigned to it."))
...
template<typename _Interface, typename _InterfacePtr> bool operator==(_In_opt_ _Interface* i, _In_opt_ _com_ptr_t<_InterfacePtr>& p) throw(_com_error)
{
	return p == i;
}

template<typename _Interface> bool operator!=(_In_opt_ _When_(null != 0, __drv_reportError("This function will issue an error if any value other than NULL is assigned to it."))
