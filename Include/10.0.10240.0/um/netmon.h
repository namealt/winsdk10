...
VOID                 _cdecl dprintf(_In_ LPSTR format, ...);
...
LPSTR        WINAPI AddressToString(_Out_writes_(MAX_ADDR_LENGTH) LPSTR string,
                                    _In_reads_(MAX_ADDR_LENGTH)  BYTE *lpAddress);

LPBYTE       WINAPI StringToAddress(_Out_writes_(MAX_ADDR_LENGTH*2) LPBYTE lpAddress,
                                    _In_reads_(MAX_ADDR_LENGTH*2) LPSTR string);
...
DWORD        WINAPIV FormatByteFlags(_Out_writes_(8) LPSTR string, DWORD ByteValue, DWORD BitMask);

DWORD        WINAPIV FormatWordFlags(_Out_writes_(16) LPSTR string, DWORD WordValue, DWORD BitMask);

DWORD        WINAPIV FormatDwordFlags(_Out_writes_(32) LPSTR string, DWORD DwordValue, DWORD BitMask);

LPSTR        WINAPIV FormatTimeAsString(SYSTEMTIME *time, _Inout_updates_(MAX_PATH) LPSTR string);
...
LPSTR _cdecl BuildINIPath( _Out_writes_(MAX_PATH) LPSTR FullPath,
                           _In_ LPSTR                   IniFileName );

// Builds Handoff Set
DWORD     WINAPI CreateHandoffTable(_In_ LPSTR          secName,
                                    _In_ LPSTR          iniFile,
...
BOOLEAN WINAPI IsRawIPXEnabled(_In_ LPSTR secName,
                               _In_ LPSTR iniFile,
                               _In_ LPSTR CurProtocol );
...
HPROTOCOL            WINAPI CreateProtocol(_In_ LPSTR ProtocolName,
...
HPROPERTY            WINAPI GetProperty(HPROTOCOL hProtocol, _In_ LPSTR PropertyName);

HPROTOCOL            WINAPI GetProtocolFromName(_In_ LPSTR ProtocolName);

DWORD                WINAPI GetProtocolStartOffset(HFRAME hFrame, _In_ LPSTR ProtocolName);
...
LPSTR                WINAPI GetPropertyText(HFRAME hFrame, LPPROPERTYINST lpPI, _Out_writes_(BufferSize) LPSTR szBuffer, DWORD BufferSize);
...
DWORD                WINAPI GetCaptureCommentFromFilename(_In_ LPSTR lpFilename, _Out_writes_(CommentSize) LPSTR lpComment, DWORD CommentSize);
...
LPSTR                WINAPI NMRtlIpv6AddressToStringA(const BYTE IP6Addr[], _Out_writes_(MAX_PATH) LPSTR S);

LPWSTR               WINAPI NMRtlIpv6AddressToStringW(const BYTE IP6Addr[], _Out_writes_(IPV6_STR_LENGTH*4) LPWSTR S);
