...
typedef enum _HEADLESS_CMD {
    ...
    HeadlessCmdPutData,
    HeadlessCmdPutWideString,
    HeadlessCmdQueryPciInformation
...
//
// HeadlessCmdPutWideString:
//   Input structure: String, A NULL-terminated wide string.
//

typedef struct _HEADLESS_CMD_PUT_WIDE_STRING {
    WCHAR String[1];
} HEADLESS_CMD_PUT_WIDE_STRING, *PHEADLESS_CMD_PUT_WIDE_STRING;
...
typedef struct _HEADLESS_RSP_QUERY_PCI_INFO {
...
} HEADLESS_RSP_QUERY_PCI_INFO, *PHEADLESS_RSP_QUERY_PCI_INFO;
...
HeadlessTerminalAddResources(
    _In_reads_bytes_(ResourceListSize) PCM_RESOURCE_LIST Resources,
    _In_ ULONG ResourceListSize,
    _In_ BOOLEAN TranslatedList,
    _Outptr_result_bytebuffer_(*NewListSize) PCM_RESOURCE_LIST *NewList,
    _Out_ PULONG NewListSize
    );

VOID
HeadlessKernelAddLogEntry(
    _In_ IN ULONG StringCode,
    _In_opt_ IN PUNICODE_STRING DriverName OPTIONAL
    );

NTSTATUS
HeadlessDispatch(
    _In_ IN  HEADLESS_CMD Command,
    _In_reads_bytes_opt_(InputBufferSize) IN  PVOID InputBuffer OPTIONAL,
    _In_ IN  SIZE_T InputBufferSize OPTIONAL,
    _Out_writes_bytes_opt_(*OutputBufferSize) OUT PVOID OutputBuffer OPTIONAL,
    _Out_opt_ OUT PSIZE_T OutputBufferSize OPTIONAL
    );
