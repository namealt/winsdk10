...
Abstract:
	...
	This is the private version of the header and includes interfaces
	that are not available to components outside of Windows.

	The true source of this (and its public cousin header file esent.h) 
	is ds\esent\published\inc\jethdr.w.
...
//		JET_bitDBUtilOptionDumpVerbose			0x10000000	// DEBUG only
...
typedef enum
	{
	JET_tracetagNull,
	JET_tracetagInformation,			// [Informational]
	JET_tracetagErrors,					// [Informational]
	JET_tracetagAsserts,				// [Error]
	JET_tracetagAPI,
...
#endif // JET_VERSION >= 0x0602
#endif // JET_VERSION >= 0x0601
	JET_tracetagMax,					//	sentinel value
	} JET_TRACETAG;
...
// end_PubEsent
// FUTURE-2007/06/29-BrettSH - Should I have just #ifdef'd the new args at the end of TABLECREATE2_W?
// begin_PubEsent
#if ( JET_VERSION >= 0x0601 )
...
typedef struct
	...
	union {
		...
		struct
			...
			// FUTURE-2009/12/05-BrettSh - Would be nice to pass the event strings?
			} NotificationEvent;
...
// Introduced in Win7, but format changed in Win10.
#if ( JET_VERSION >= 0x0A00 )
...
// ISSUE-2007/06/25-BrettSh - After a review of what this is, I'd like to
//	consider renaming these next two variables to pInternalPageSpaceInfo and 
//	pDataPageSpaceInfo respectively?  Just a thought.
...
#define JET_IOPriorityLowForCacheShrink			0x8
#endif // JET_VERSION >= 0x0603
// begin_PubEsent
...
// Database Shrink is cut for Exchange 15. It can still be enabled via a test hook.
// ENABLE_DATABASE_SHRINK // A text tag to search for, since there isn't an ifdef here.
//
#define JET_paramAutomaticShrinkDatabaseFreeSpaceThreshold	185 //	Minimum threshold (percentage of the database size) that determines if the periodic shrink and/or shrink at JetTerm will take place or not.
...
// end_PubEsent
#define JET_bitShrinkDatabaseEof			0x4000	// Releases free space at the end of the file. Does not rearrange data.
#define JET_bitShrinkDatabasePeriodically	0x8000	//	Periodically try to shrink the database.
// begin_PubEsent
...
typedef JET_ERR (JET_API * JET_PFNEMITLOGDATA)(
	...
	/* Flags for JetConsumeLogData */

#define JET_bitShadowLogConsumeFlushData		0x00000001	//	write the log data to disk
...
// end_PubEsent
// FUTURE-2009/08/03-BrettSh - Remove these unpublic constants from Windows.
...
// Database Shrink is cut for Exchange 15. It can still be enabled via a test hook.
// ENABLE_DATABASE_SHRINK // A text tag to search for, since there isn't an ifdef here.
#define JET_bitResizeDatabaseShrinkCompactSize		0x00000004	// Shrink the database to the smallest possible size without keeping an empty extent at the end.
// begin_PubEsent
...
// These bits are used for private staging, alpha, beta, and test in production staging.  Should not be used by
...
#define wrnDiskGameOn						4007 /* the respective IO dispatch queue has gotten low */
...
// FUTURE-2006/01/22-BrettSh defining a A() version for now, as there are exch consumers.  Be good to
// remove this and move them to W() version.
...
typedef enum
	...
	opTestHookTraceTestMarker,							//	traces a test marker ETW event
	...
	opTestHookEnableAutoInc,							//	takes a DWORD*
...
enum
	{
	fDeletingLogFiles					= 0x01,
	fCorruptingLogFiles					= 0x02,
	fLockingCheckpointFile				= 0x04,
	fCorruptingDbHeaders				= 0x08,
	fCorruptingPagePgnos				= 0x10,	// but checksum is ok.
	fLeakStuff							= 0x20,
	fCorruptingWithLostFlush			= 0x40,
	fDisableTimeoutDeadlockDetection	= 0x80,
	fCorruptingPages					= 0x100,
	fDiskIOError						= 0x200, 	// Injecting ERROR_IO_DEVICE (results in Jet_errDiskIO)
	fInvalidAPIUsage					= 0x400,	// invalid usage of the JET API
...
#define JET_bitInjectionProbabilityPermanent	0x00000004	//  same as one shot, but permanent after fire (must be XOR'd with JET_bitInjectionProbabilityCount).
#define JET_bitInjectionProbabilityFailUntil	0x00000008	//  same as one shot, but fails until fire (must be XOR'd with JET_bitInjectionProbabilityCount).
...
#define JET_bitTestHookCorruptSizeLargerThanNode	0x00010000	//	usable only with JET_bitTestHookCorruptNodePrefix | JET_bitTestHookCorruptNodeSuffix	
#define JET_bitTestHookCorruptSizeShortWrapSmall	0x00020000	//	usable only with JET_bitTestHookCorruptNodePrefix | JET_bitTestHookCorruptNodeSuffix	
#define JET_bitTestHookCorruptSizeShortWrapLarge	0x00040000	//	usable only with JET_bitTestHookCorruptNodePrefix | JET_bitTestHookCorruptNodeSuffix	
