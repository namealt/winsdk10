...
#define _KERNEL_MODE
    ...
    #ifdef _USE_ETW_
        #include <evntprov.h>
        #include <Microsoft-WindowsPhone-Sensors_UM.h>
    #endif
...
#define MAX_STRING_LEN 256
    ...
    if ((DebugLevel >= 0) && (DebugLevel <= (sizeof (g_DebugZones) / sizeof (DWORD))))
    {
...
#else /* USER MODE*/
            vswprintf_s(wBuffer, sizeof(wBuffer)/sizeof(wBuffer[0]), DebugMessage, ArgList);
            if (DebugZone & DBGZ_PROFILE)
            {
                EventWriteSensorPerf (wBuffer);
            }
            if (DebugLevel == dlError)
            {
                EventWriteSensorError(wBuffer);
            }
            // For Error: use EventWriteSensorError(&WpSensorsEtwGuid, wBuffer);
            // For anything else: use EventWriteSensorDebug(&WpSensorsEtwGuid, wBuffer);
#endif /*_KERNEL_MODE*/
