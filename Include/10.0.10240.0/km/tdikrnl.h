...
// Chained receives are not available on new platforms
...
NTSTATUS
(*PTDI_IND_ERROR)(
    _In_opt_ PVOID TdiEventContext,    // the endpoint's file object.
    _In_ NTSTATUS Status               // status code indicating error type.
    );
...
(*PTDI_IND_RECEIVE)(
    ...
    _In_ PVOID Tsdu,                   // pointer describing this TSDU, typically a lump of bytes
    _Out_opt_ PIRP *IoRequestPacket    // TdiReceive IRP if MORE_PROCESSING_REQUIRED.
    );
...
(*PTDI_IND_RECEIVE_DATAGRAM)(
    _In_opt_ PVOID TdiEventContext, // the event context
    _In_ LONG SourceAddressLength, // length of the originator of the datagram
    _In_reads_bytes_(SourceAddressLength) PVOID SourceAddress, // originator of the datagram
    _In_ LONG OptionsLength, // options for the receive
    _In_reads_bytes_opt_(OptionsLength) PVOID Options, //
    _In_ ULONG ReceiveDatagramFlags, //
    _In_ ULONG BytesIndicated, // number of bytes this indication
    _In_ ULONG BytesAvailable, // number of bytes in complete Tsdu
    _Out_ ULONG *BytesTaken, // number of bytes used
    _In_ PVOID Tsdu, // pointer describing this TSDU, typically a lump of bytes
    _Out_opt_ PIRP *IoRequestPacket // TdiReceive IRP if MORE_PROCESSING_REQUIRED.
    );
...
(*PTDI_IND_RECEIVE_EXPEDITED)(
    ...
    _In_opt_ CONNECTION_CONTEXT ConnectionContext,
    _In_ ULONG ReceiveFlags, //
    _In_ ULONG BytesIndicated, // number of bytes in this indication
    _In_ ULONG BytesAvailable, // number of bytes in complete Tsdu
    _Out_ ULONG *BytesTaken, // number of bytes used by indication routine
    _In_ PVOID Tsdu, // pointer describing this TSDU, typically a lump of bytes
    _Out_opt_ PIRP *IoRequestPacket // TdiReceive IRP if MORE_PROCESSING_REQUIRED.
    );
...
(*PTDI_IND_CHAINED_RECEIVE)(
    ...
    _In_opt_ CONNECTION_CONTEXT ConnectionContext,
    _In_ ULONG ReceiveFlags,
    _In_ ULONG ReceiveLength, // length of client data in TSDU
    _In_ ULONG StartingOffset, // offset of start of client data in TSDU
    _In_ PMDL  Tsdu, // TSDU data chain
    _In_ PVOID TsduDescriptor // for call to TdiReturnChainedReceives
    );
...
(*PTDI_IND_CHAINED_RECEIVE_DATAGRAM)(
    ...
    _In_reads_bytes_opt_(OptionsLength) PVOID Options, //
    _In_ ULONG ReceiveDatagramFlags,  //
    _In_ ULONG ReceiveDatagramLength, // length of client data in TSDU
    _In_ ULONG StartingOffset, // offset of start of client data in TSDU
    _In_ PMDL  Tsdu, // TSDU data chain
    _In_ PVOID TsduDescriptor // for call to TdiReturnChainedReceives
    );
