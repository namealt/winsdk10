...
// Do not force clients to unnecessarily include msxml, use IUnknown if its not included
        ...
        _Out_ IXMLDOMElement** pAnalysisXml
#else 
        _Out_ IUnknown** pAnalysisXml
#endif
        ) PURE;

    // Adds another analysis object as structured data
    // in a new entry
...
(WINAPI* EXT_GET_DEBUG_FAILURE_ANALYSIS)(
    ...
    _In_ ULONG Flags,
    _In_ CLSID pIIdFailureAnalysis,     // must be IID_IDebugFailureAnalysis2
    _Out_ PDEBUG_FAILURE_ANALYSIS2* Analysis
    );
