...
void
SanitizeFileName(_Inout_updates_(FileNameLen) PSTR FileName, DWORD FileNameLen)
{
...
void
GetDumpPath(_In_ PSTR NameQual, _In_ PSTR TypeStr, _Out_writes_(PathLen) PSTR Path, DWORD PathLen)
{
    SYSTEMTIME Time;
    PSTR FilePart;
    ...
    _snprintf_s(FilePart,
              PathLen - (FilePart - Path),
              _TRUNCATE,
              "PID-%u__%s__Date_%02u-%02u-%04u__Time_%02u-%02u-%02u__%s__%s.dmp",
              g_ProcessId,
              g_ProcessName,
              Time.wMonth,
              Time.wDay,
    ...
    SanitizeFileName(FilePart, (DWORD)(PathLen - (FilePart - Path)));
}
            
void
WriteDump(_In_ PSTR NameQual, _Inout_updates_(MAX_COMMENT) PSTR Comment,
          ULONG DumpQual, ULONG DumpFormat, _In_ PSTR TypeStr)
{
    char Path[MAX_PATH];
    size_t Len;
