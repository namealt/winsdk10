...
class GdbSrvController::GdbSrvControllerImpl
{
    ...
    {
        for (unsigned i = 0; i < coreNumberConnectionParameters.size(); ++i)
        {
    ...
    TARGET_HALTED GdbSrvControllerImpl::ReportReasonTargetHalted(_Out_ StopReplyPacketStruct * pStopReply)
    {
        ...
        for (unsigned core = 0; core < numberOfCoreConnections; ++core)
        {
            ...
            if (HandleAsynchronousCommandResponse(cmdResponse, &coreStopReply))
            {
                m_TargetHaltReason = coreStopReply.stopReason;
                if (coreStopReply.status.isTAAPacket && coreStopReply.status.isThreadFound)
                {
                    if (coreStopReply.processorNumber != 0 && coreStopReply.processorNumber != static_cast<ULONG>(C_ALLCORES))
                    {
    ...
    bool GdbSrvControllerImpl::IsTargetHalted()
    {
        ...
        return (haltReason == TARGET_BREAK_SIGTRAP);    
    }
                                                                            ...
                                                                            _In_ const size_t numberOfElements) 
    {
        ...
        for (size_t index = 0; index < numberOfElements; ++index)
        {
            ...
            assert((pRegEntry->registerSize << 1) == reply.length());
    ...
    SimpleCharBuffer GdbSrvControllerImpl::ReadMemory(_In_ AddressType address, _In_ size_t maxSize)
    {
        ...
        {   
            ...
            for (;;)
            {
                ...
                PCSTR pFormat = (m_targetProcessorArch == ARM64_ARCH || m_targetProcessorArch == AMD64_ARCH) ?
                                 "m%I64x,%x" : "m%x,%x";     
                                           ...
                                           _Out_ DWORD * pdwBytesWritten)
    {
        ...
        for (;;)
        {
            ...
            char dataLength[128];
            sprintf_s(dataLength, _countof(dataLength), "%x", maxPacketSize);
...
SimpleCharBuffer GdbSrvController::ReadMemory(_In_ AddressType address, _In_ size_t size)
{
    assert(m_pGdbSrvControllerImpl != nullptr);
    return m_pGdbSrvControllerImpl->ReadMemory(address, size);
}
                                   ...
                                   _Out_ DWORD * pdwBytesWritten)
{
    ...
    return m_pGdbSrvControllerImpl->WriteMemory(address, size, pRawBuffer, pdwBytesWritten);
}
