    ...
    SAFEARRAY * *pbReadBuffer)
{
    try
    {
        ...
        SimpleCharBuffer buffer = pController->ReadMemory(Address, dwBytesToRead);
        ...
        /* [out] */ DWORD *pdwBytesWritten)
{
    try
    {
        ...
        bool isWriteDone = pController->WriteMemory(Address, bufferSize, pRawBuffer, pdwBytesWritten);
        if (isWriteDone)
        {
    	    return S_OK;
        }
...
HRESULT STDMETHODCALLTYPE CLiveExdiGdbSrvSampleServer::OnAsynchronousCommandCompleted()
{
    if (m_pRunNotificationListener != nullptr)
    {
        ...
        DWORD eventProcessor = 0;
        ADDRESS_TYPE currentAddress = ParseAsynchronousCommandResult(&eventProcessor);
        if (currentAddress != 0)
        {
...
DWORD CALLBACK CLiveExdiGdbSrvSampleServer::NotificationThreadBody(LPVOID p)
{
    ...
    DWORD waitTimeout = (cfgData.GetMultiCoreGdbServer()) ? INFINITE : 1000;  
...
ADDRESS_TYPE CLiveExdiGdbSrvSampleServer::ParseAsynchronousCommandResult(_Out_ DWORD * pProcessorNumberOfLastEvent)
{
    assert(pProcessorNumberOfLastEvent != nullptr);
    AsynchronousGdbSrvController * pController = GetGdbSrvController();
    assert(pController != nullptr);
        ...
        if (isParsed)
        {
            ...
            if (stopReply.status.isTAAPacket)
            {
                ...
                if (stopReply.status.isThreadFound)
                {
                    ...
                    assert(stopReply.processorNumber != 0 && stopReply.processorNumber != static_cast<ULONG>(-1));
